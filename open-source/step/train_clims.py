...
from clip_utils import clip_forward, category_dict
from clip_loss import SimMaxLoss, SimMinLoss, BackgroundSuppressionLoss, InfoNCELoss
...
def run(args):
    ...
    # bgs
    bg_file_path = '../voc12/voc12_bgs.npy'

    bgs: dict = np.load(bg_file_path, allow_pickle=True).item()  
    print('bg file loaded from {}'.format(bg_file_path))
    ...
    NCELoss = InfoNCELoss(temperature=args.nce_t, bg_bg_pos=args.nce_bg_bg, alpha=args.nce_alpha, beta=args.nce_beta)
    print(hyper)
    ...
    for ep in range(args.clims_num_epoches):
        ...
        for step, pack in enumerate(train_data_loader):
            ...
            fg_labels = [category_dict['voc'][_] for _ in fg_label_idxes]
            fg_image_name_idxes = fg_indices.cpu().numpy() // len(category_dict['voc'])
            fg_image_names = [img_names[_] for _ in fg_image_name_idxes]
            fg_image_bg_labels = [bgs[_im][_lb]['bgs'] for _im, _lb in zip(fg_image_names, fg_labels)]
            fg_image_bg_labels = [list(set(_)) for _ in fg_image_bg_labels]
            ...
            L_NCE = NCELoss(clip_model, fg_224_eval, fg_labels, bg_224_eval, fg_image_bg_labels)

            L_REG = torch.mean(x)

            # loss = hyper[0] * L_OTM + hyper[1] * L_BTM + hyper[2] * L_CBS + hyper[3] * L_REG
            # loss = hyper[0] * L_OTM + hyper[1] * L_BTM + hyper[2] * L_NCE + hyper[3] * L_REG
            loss = hyper[2] * L_NCE + hyper[3] * L_REG

            loss.backward()
            optimizer.step()

            avg_meter.add({'loss1': loss.item(),
                           # 'L_OTM': L_OTM.item(), 'L_BTM': L_BTM.item(),
                           # 'L_CBS': L_CBS.item(),
                           'L_NCE': L_NCE.item(),
                           'L_REG': L_REG.item()})
